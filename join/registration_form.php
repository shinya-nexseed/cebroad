...
if(empty($_GET)) {
	...
	if ($urltoken == ''){
		...
		try{
			...
			$statement = $dbh->prepare("SELECT mail FROM pre_member WHERE urltoken=(:urltoken) AND flag =0 AND created > now() - interval 24 hour");
...
<p>email：<?=htmlspecialchars($mail, ENT_QUOTES, 'UTF-8')?></p>
<p>name：<input type="text" name="account"></p>
